...
function drawPath(player, destPos)
  local color = {0,1,1}

  local _, pdata = Player.get(player.index)

  for _, id in pairs(pdata.sprites or {}) do
  ...
  for _,belt in pairs(rv.belts) do
    sprites[#sprites+1] = rendering.draw_line{color=color, width=8, from=belt.pos, to=Pos.add(belt.pos, Dir.toOffset[dir]), surface = player.surface}
  end
  pdata.sprites = sprites
end

local modIsPlacing = false
script.on_event(defines.events.on_built_entity, function(event)
  -- Ignore events generated by this mod itself.
  if modIsPlacing then return end
  ...
  if event.created_entity.name == "entity-ghost" then
    ...
    if lastBelt then
      ...
      dir = Dir.getPrimary(lastBelt.pos, pos)
